...
do
 for n in $num
 do
  output_warning_type=`cat $jaso_file | jq '.warnings' | jq ".[$n]"`
  ...
  git_project_path=`cat $jaso_file | grep app_path | awk -F'"' '{print $4}'`
...
echo $git_branch
  ###update repo info###
  repo=`cat $jaso_file | grep app_path | awk -F'"' '{print $4}' | awk -F'/' '{print $4}'`
